...
def synth_segment_to_duration(
    ...
    if sr_out and sr != sr_out:
        # Resample to match final stream if needed
...
def build_dubbed_track_from_segments(
    ...
    """
    # We'll use the XTTS default SR from the first segment
    ...
    for (start, end), text_hi in zip(seg_times, seg_text_hi):
        ...
        if sr_global is None:
            # First synth dictates global SR
...
def pipeline_translate_dub(job: Job):
    ...
    seg_text_hi: List[str] = []
    # translate each segment (chunk internally if needed)
...
async def on_start(message: Message):
    await message.answer(
        "👋 Send me a short English video (mp4/mov). Tap **Translate EN→HI (same voice)**.\n\n"
        "I’ll transcribe → translate → clone the voice → replace audio.\n"
        "Tip: Clear speech & ≤3 minutes gives best results."
    )


@dp.message()
async def on_video(message: Message):
    # Only react to videos/documents that are videos
    is_video = False
    if message.video:
        is_video = True
