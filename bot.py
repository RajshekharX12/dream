...
# - Antilink (invite links), soft antiflood (window counter)
...
# - Police: deep system snapshot + concise one-liner via SafoneAPI (fallback safe)
# Only this file + Python 3.7+ required.

# -----------------------
# 0) Auto-install deps (safe for 3.7+)
# -----------------------
...
# -----------------------
# 2) CONFIG (your token hardcoded)
# -----------------------
BOT_TOKEN = "8287015753:AAGoGYF_u6-OqfrqGF1_xPY8yIW5FiD9MtE"  # <-- your token
if not BOT_TOKEN:
    raise SystemExit("No BOT_TOKEN provided!")
...
# -----------------------
# 3) In-memory stores (no DB)
# -----------------------
GROUPS = {ChatType.GROUP, ChatType.SUPERGROUP}
ADMIN_STATUSES = {"creator", "administrator"}

# Per-chat settings (defaults populated lazily)
SETTINGS = {}              # type: dict[int, dict]
WARNS = {}                 # type: dict[Tuple[int,int], int]
NOTES = {}                 # type: dict[int, dict[str,str]]
RULES = {}                 # type: dict[int, str]
LOCKS = {}                 # type: dict[int, dict[str,bool]]
FLOOD_BUCKET = {}          # type: dict[Tuple[int,int], list[float]]
...
def parse_duration_utc(text: Optional[str], default_minutes: Optional[int]) -> Tuple[datetime, bool]:
    """Return (until_date, is_perm). >366 days ≈ permanent. If no duration and default_minutes is None => permanent."""
    t = text or ""
    if PERM_RE.search(t):
    ...
    return datetime.now(timezone.utc) + timedelta(**{kw: n}), False

def fmt_dur(until: Optional[datetime], is_perm: bool) -> str:
    if is_perm or until is None:
        return "permanent"
    return f"until <code>{until:%Y-%m-%d %H:%M UTC}</code>"

def strip_reason(text: Optional[str], keyword: str) -> str:
    if not text:
        return ""
...
def _cpu_percent_sample(interval: float = 0.2) -> float:
    def read():
        with open("/proc/stat", "r") as f:
            line = f.readline()
        parts = [int(x) for x in line.strip().split()[1:8]]
...
def _swap_info_mb():
    try:
        ...
        def val(k):
            m = re.search(rf"^{k}:\s+(\d+)\s+kB", data, re.M)
            return int(m.group(1)) if m else 0
        total = val("SwapTotal")//1024
        free  = val("SwapFree")//1024
        used  = total - free
        return used, total
    except Exception:
        return 0,0
...
def _loadavg():
    try:
        ...
        return round(a1,2), round(a5,2), round(a15,2)
    except Exception:
        return 0.0, 0.0, 0.0
...
async def warn_text(m: Message):
    ...
    if cnt >= 3:
        ...
        out = await m.reply(f"⚠️ Warning {cnt}/3 for {mention(target)}\n👮 By: {mention(m.from_user)}\n📝 Reason: {h(reason or '—')}")
        asyncio.create_task(autodel(m, 3)); asyncio.create_task(autodel(out, 6))

# -----------------------
# 9) Manage menu (inline UI)
# -----------------------
...
await bot.restrict_chat_member(
    chat.id,
    target_id,
    permissions=NO_SPEAK,
    until_date=None if is_perm else until
)
