...
from aiogram.types import Message, CallbackQuery, FSInputFile, BotCommand, Update
from aiogram.utils.keyboard import InlineKeyboardBuilder
from aiogram.exceptions import TelegramBadRequest # Added for status updates

# AI dependencies (imported here but used inside functions for lazy loading)
...
# ==================== CONFIG ====================
# FIX 6: Use explicit path for load_dotenv to avoid AssertionError
...
def run_ffprobe_get_duration(path: str) -> float:
    # Use pipe=True to handle the output more robustly
...
# ==================== ASR (faster-whisper) ====================
# from faster_whisper import WhisperModel # Moved to top imports
# import torch # Moved to top imports
...
# ==================== Translation (en->hi) ====================
# from transformers import pipeline as hf_pipeline # Moved to top imports
_TRANSLATOR = None
...
# from TTS.api import TTS # Moved to top imports
...
def build_kb(st: SessionState):
    ...
    kb.adjust(1)
    # FIX 4: Ensure .as_markup() is returned for InlineKeyboardMarkup object
    return kb.as_markup()
...
async def update_status(m: Message, st: SessionState, text: str):
    ...
    except TelegramBadRequest: # Catch edit errors (e.g., message not modified)
        pass 
    except Exception:
        pass

async def finalize_status(m: Message, st: SessionState, success: bool = True):
    """Updates the final status message and clears the ID."""
    try:
        if st.status_msg_id:
            text = "✅ Done! All tasks completed." if success else "❌ Job ended with an error."
...
async def errors_handler(update: Update, exception: Exception) -> bool:
    """Catches and reports all unhandled errors."""
    log(f"--- UNHANDLED EXCEPTION ---")
    log(f"Update: {update.model_dump_json(indent=2)}")
    log(f"Error Type: {type(exception).__name__}")
    log(f"Error Message: {exception}")
    import traceback
    log(traceback.format_exc())

    # Try to send a simple error message to the user
    # Check if a message exists in the update
    message = update.message or update.callback_query.message if update.callback_query else None
    if message:
        try:
            await bot.send_message(
                chat_id=message.chat.id,
                text=f"❌ **Error:** An unhandled error occurred (Type: {type(exception).__name__}). Check the console logs.",
                parse_mode="Markdown"
            )
        except Exception:
            pass 

    # Returning True means the error should NOT be propagated up and should be considered handled
    return True 
...
async def start_cmd(m: Message):
    ...
    # FIX 2: Corrected multi-line string by ensuring triple quotes or a single-line quote.
    await m.answer(
...
async def on_video(m: Message):
    ...
    # Ensure a clean slate before starting a new job
